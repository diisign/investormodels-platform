...
export const generateMonthlyPerformanceData = (creatorId: string) => {
  ...
  const revenueValues = monthNames.map((_, index) => {
    ...
    // For August (index 12, the new last month), create mostly positive variations from June's value
    if (index === 12) {
      ...
      const augustRevenue = Math.round(monthlyRevenue * (1 + variationPercent / 100));
...
// Fonction pour obtenir le Market Cap de manière cohérente
export const getMarketCap = (creatorId: string, creators?: any[]): number => {
  // Chercher d'abord dans mockData si fourni
  if (creators) {
