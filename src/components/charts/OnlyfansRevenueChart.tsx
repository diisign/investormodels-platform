...
const OnlyfansRevenueChart: React.FC<OnlyfansRevenueChartProps> = ({
  data
}) => {
  ...
  const withdrawalPoint = isMonthlyData ? chartData.findIndex(item => 'withdrawal' in item && item.withdrawal && item.withdrawal > 0) : -1;
      ...
      <div className="relative bg-white dark:bg-gray-900 rounded-2xl shadow-xl overflow-hidden border-2 border-black dark:border-black p-2 sm:p-4 h-full">
        ...
        <AreaChart data={chartData} margin={{
          ...
          bottom: 5
            ...
            left: 25,
            right: 20
          }} tickMargin={5} interval={0} />
              <YAxis tick={{
            fill: 'var(--foreground)',
            fontSize: 10
          }} tickLine={{
            stroke: 'var(--border)'
          }} width={40} dx={3} domain={[0, 10000]} ticks={fixedTicks} allowDecimals={false} />
            ...
            if (active && payload?.length) {
              return <ChartTooltipContent active={active} payload={payload} label={label} formatter={formatTooltipValue} />;
            }
            return null;
          }} />
              <Area type="stepAfter" dataKey="invested" stroke="#0ea5e9" strokeWidth={3} fillOpacity={1} fill="url(#investedGradient)" name="Investissement" stackId="1" />
              <Area type="stepAfter" dataKey="return" stroke="#22c55e" strokeWidth={3} fillOpacity={1} fill="url(#returnGradient)" name="Rendement" />
        ...
        <AreaChart data={chartData} margin={{
          ...
          bottom: 5
            ...
            stroke: 'var(--border)'
          }} tickFormatter={value => value < 1 ? `${value * 1000}M` : `${value}B`} width={40} dx={3} ticks={[0, 0.4, 0.8, 1.2, 1.6]} />
              <ChartTooltip content={({
            active,
            payload,
            label
            ...
            return null;
              ...
              <Area type="stepAfter" dataKey="revenue" stroke="#eab308" strokeWidth={3} fillOpacity={1} fill="url(#revenueGradient)" name="Chiffre d'affaires" />
              <Legend />
            </AreaChart>}
        </ChartContainer>
