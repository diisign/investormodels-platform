...
interface DashboardStatsSpecialProps {
...
const DashboardStatsSpecialBackup = ({
  totalInvested,
  totalReturn,
  investmentsCount,
  balance,
  onDepositClick
}: DashboardStatsSpecialProps) => {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-4 sm:mb-6">
      ...
      <FadeIn direction="up" delay={300}>
        <Card className="p-4 sm:p-5 hover:bg-gray-50/80 dark:hover:bg-gray-800/60 transition-colors">
          <div className="flex items-center justify-between mb-2 sm:mb-3">
            <h3 className="text-sm font-medium text-gray-600 dark:text-gray-400">Bénéfices</h3>
            <div className="h-8 w-8 flex items-center justify-center rounded-lg bg-blue-100 dark:bg-blue-900/30 text-blue-600">
              <TrendingUp className="h-4 w-4" />
            </div>
          </div>
          <div className="flex items-end">
            <span className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-white">36,789€</span>
          </div>
        </Card>
      </FadeIn>
...
export default DashboardStatsSpecialBackup;