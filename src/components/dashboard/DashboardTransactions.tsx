      ...
      <div className="p-6 my-0 py-[2px] px-0 mx-0">
        ...
        {transactions.length > 0 ? (
          <div className="h-80 overflow-x-auto overflow-y-hidden">
            <div className="flex gap-4 pb-4" style={{ width: `${Math.max(transactions.length * 280, 100)}px` }}>
              {transactions.map(transaction => {
                const type = transaction.payment_method === 'investment' ? 'investment' : transaction.amount > 0 ? 'deposit' : 'withdrawal';
                return (
                  <div key={transaction.id} className="flex-shrink-0 w-64 flex flex-col p-3 rounded-lg border border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/50">
