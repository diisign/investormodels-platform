...
const DashboardTransactions = ({
  ...
  return <FadeIn direction="up" delay={100}>
      <div className="p-6 my-0 py-[2px] px-0 mx-0">
        ...
        {transactions.length > 0 ? <div className="overflow-x-auto pb-2">
            <div className="flex space-x-4 min-w-max">
              {transactions.map(transaction => {
                const type = transaction.payment_method === 'investment' ? 'investment' : transaction.amount > 0 ? 'deposit' : 'withdrawal';
                return <div key={transaction.id} className="flex-shrink-0 w-72 flex items-center p-3 rounded-lg border border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/50">
                        ...
                        <div className="flex-grow min-w-0">
                          <div className="flex justify-between items-center">
                            ...
                            <span className={cn("text-sm font-semibold ml-2", type === 'deposit' ? "text-primary" : type === 'withdrawal' ? "text-primary" : "text-primary")}>
                              {type === 'deposit' ? '+' : ''}
                          ...
                          <div className="flex justify-between items-center mt-1">
                            ...
                            <span className={cn("text-xs px-2 py-0.5 rounded-full ml-2", transaction.status === 'completed' ? "bg-primary/10 text-primary dark:bg-primary/20 dark:text-primary" : transaction.status === 'pending' ? "bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400" : "bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400")}>
