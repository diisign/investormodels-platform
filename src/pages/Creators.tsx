...
const Creators = () => {
  ...
  const filteredCreators = allCreators.filter(creator => {
    const matchesSearch = creator.name.toLowerCase().includes(searchTerm.toLowerCase()) || creator.category.toLowerCase().includes(searchTerm.toLowerCase());
              ...
              <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md border border-gray-100 dark:border-gray-700 p-4">
                <div className="flex flex-col md:flex-row gap-4">
                  <div className="relative flex-grow">
                    ...
                    <input type="text" placeholder="Rechercher une créatrice..." value={searchTerm} onChange={handleSearchChange} className="input-field pl-10 w-full" />
                  </div>
                  
                  <div className="flex gap-2">
                    <div className="relative">
                      <button className="bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 px-4 py-2.5 rounded-lg flex items-center hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors" onClick={toggleFilters}>
                        <Filter className="h-5 w-5 mr-2" />
                        <span>Filtres</span>
                      </button>
                      
                      {showFilters && <div className="absolute right-0 mt-2 w-64 bg-white dark:bg-gray-800 rounded-lg shadow-lg overflow-hidden border border-gray-100 dark:border-gray-700 z-10 animate-scale-in origin-top-right">
                          <div className="p-4">
                            <h4 className="font-semibold mb-3">Trier par</h4>
                            <div className="space-y-2">
                              <button className={cn("w-full text-left px-3 py-2 rounded-lg flex items-center", sortBy === 'popularity' ? "bg-investment-100 dark:bg-investment-900/30 text-investment-600 dark:text-investment-400" : "hover:bg-gray-100 dark:hover:bg-gray-700")} onClick={() => handleSortChange('popularity')}>
                                <Users className="h-4 w-4 mr-2" />
                                <span>Popularité</span>
                              </button>
                              <button className={cn("w-full text-left px-3 py-2 rounded-lg flex items-center", sortBy === 'return' ? "bg-investment-100 dark:bg-investment-900/30 text-investment-600 dark:text-investment-400" : "hover:bg-gray-100 dark:hover:bg-gray-700")} onClick={() => handleSortChange('return')}>
                                <TrendingUp className="h-4 w-4 mr-2" />
                                <span>Rendement</span>
                              </button>
                              <button className={cn("w-full text-left px-3 py-2 rounded-lg flex items-center", sortBy === 'alphabetical' ? "bg-investment-100 dark:bg-investment-900/30 text-investment-600 dark:text-investment-400" : "hover:bg-gray-100 dark:hover:bg-gray-700")} onClick={() => handleSortChange('alphabetical')}>
                ...
                {(searchTerm || sortBy !== 'return') && <div className="flex flex-wrap gap-2 mt-4">
                    {sortBy !== 'return' && <div className="bg-investment-100 dark:bg-investment-900/30 text-investment-600 dark:text-investment-400 text-sm rounded-full px-3 py-1 flex items-center">
                        <span>Tri: {sortBy === 'popularity' ? 'Popularité' : sortBy === 'alphabetical' ? 'Alphabétique' : 'Rendement'}</span>
                        <button className="ml-2 hover:text-investment-800" onClick={() => handleSortChange('return')}>
                          &times;
                        </button>
                      </div>}
                    {searchTerm && <div className="bg-investment-100 dark:bg-investment-900/30 text-investment-600 dark:text-investment-400 text-sm rounded-full px-3 py-1 flex items-center">
                        <span>Recherche: {searchTerm}</span>
                        <button className="ml-2 hover:text-investment-800" onClick={() => setSearchTerm('')}>
                          &times;
                        </button>
                      </div>}
                    <button className="text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300 text-sm" onClick={() => {
                  setSearchTerm('');
                  setSortBy('return');
                }}>
