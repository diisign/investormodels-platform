...
// Données statiques simplifiées - 2 parrainages seulement
const STATIC_REFERRALS: StaticReferral[] = [
  { name: 'Marie L.', date: '28/05/2025', status: 'completed', reward: 75 },
  { name: 'Thomas B.', date: '31/05/2025', status: 'completed', reward: 150 }
];
...
export const useStaticReferralData = () => {
  return useMemo((): StaticReferralData => {
    ...
    return {
      ...
      tierProgress: Math.round((completed.length / 10) * 100),
      currentTier: 'Bronze',
      nextTier: 'Silver',
